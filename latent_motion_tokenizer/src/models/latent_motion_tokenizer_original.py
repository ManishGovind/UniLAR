...
class LatentMotionTokenizer(nn.Module):
    ...
    def forward(self, cond_pixel_values=None, target_pixel_values=None, cond_depth_values=None,target_depth_values=None,
        ...
        # Tokenization
        with torch.no_grad():
            if cond_pixel_values is not None and target_pixel_values is not None :
                # print("from rgb")
                cond_hidden_states = self.image_encoder(cond_pixel_values).last_hidden_state
                target_hidden_states = self.image_encoder(target_pixel_values).last_hidden_state
            if cond_depth_values is not None and  target_depth_values is not None : 
                # print("from depth")
                cond_hidden_states = self.image_encoder(cond_depth_values).last_hidden_state
                target_hidden_states = self.image_encoder(target_depth_values).last_hidden_state
        ...
        # print("depth maps reconstructed ", recons_depth_values.shape)
